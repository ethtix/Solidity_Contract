...
contract NFTTicketAutoId is ERC721Pausable {
    ...
    function safeMint(address _to) external onlyMinter {
        _safeMint(_to, _currentNextId);
        _currentNextId += 1;
    }
    
    // Batch mint tokens. Assign directly to _to[].
    function safeBatchMint(address[] calldata _to) external onlyMinter {
        //require(msg.sender == creators[_id]);
        for (uint256 i = 0; i < _to.length; ++i) {
            
            _safeMint(_to[i], _currentNextId + i);
        }
        _currentNextId += _to.length;
    }
